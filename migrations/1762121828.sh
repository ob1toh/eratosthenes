...
# https://github.com/ob1toh/eratosthenes/issues/1852
...
eratosthenes-pkg-add xdg-terminal-exec
...
if command -v alacritty > /dev/null 2>&1; then
  cp "$ERATOSTHENES_PATH/applications/Alacritty.desktop" ~/.local/share/applications/
fi
...
if [[ -f $waybar_config ]]; then
  sed -i 's|"on-click-right": "eratosthenes-launch-terminal"|"on-click-right": "xdg-terminal-exec"|' "$waybar_config"
  sed -i 's|"on-click": "\$TERMINAL -e btop"|"on-click": "xdg-terminal-exec btop"|' "$waybar_config"
  sed -i 's|"on-click": "\$TERMINAL --class=Wiremix -e wiremix"|"on-click": "xdg-terminal-exec --app-id=com.eratosthenes.Wiremix -e wiremix"|' "$waybar_config"
  eratosthenes-state set restart-waybar-required
fi

# Update hyprland window rules to use DNS-format class names
system_conf=~/.config/hypr/apps/system.conf
if [[ -f $system_conf ]]; then
  if grep -q 'class:(.*|Impala|' "$system_conf" || grep -q 'class:(.*|Wiremix|' "$system_conf" || grep -q '|Eratosthenes|' "$system_conf"; then
    sed -i 's/\bImpala\b/com.eratosthenes.Impala/g; s/\bWiremix\b/com.eratosthenes.Wiremix/g; s/|Eratosthenes|/|com.eratosthenes.Eratosthenes|/g' "$system_conf"
  fi
fi
