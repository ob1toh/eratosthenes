echo "Migrate Hyprland user config from Eratosthenes hyprlang files to Lua files"

hypr_config_dir="$HOME/.config/hypr"
eratosthenes_hypr_config_dir="$ERATOSTHENES_PATH/config/hypr"
toggle_state_dir="$HOME/.local/state/eratosthenes/toggles/hypr"
toggle_defaults_dir="$ERATOSTHENES_PATH/default/hypr/toggles"
timestamp=$(date +%s)
...
for lua_config in "$eratosthenes_hypr_config_dir"/*.lua "$eratosthenes_hypr_config_dir"/.luarc.json; do
...
if [[ -f $sddm_hypr_conf || -f $sddm_wayland_conf ]]; then
  if [[ -f $ERATOSTHENES_PATH/default/sddm/hyprland.lua ]]; then
    sudo mkdir -p /usr/share/sddm
    sudo cp "$ERATOSTHENES_PATH/default/sddm/hyprland.lua" "$sddm_hypr_lua"
  fi
