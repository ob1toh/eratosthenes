...
# Users can still opt-in to Cloudflare DNS using: eratosthenes-setup-dns cloudflare
...
if [[ -f /etc/systemd/resolved.conf ]]; then
  ...
  if [[ -f /etc/systemd/network/99-eratosthenes-dns.network ]]; then
    sudo rm -f /etc/systemd/network/99-eratosthenes-dns.network
  ...
  echo "To use Cloudflare DNS, run: eratosthenes-setup-dns Cloudflare"
fi