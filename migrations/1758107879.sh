...
for pkg in "${PACKAGES[@]}"; do
  if ! eratosthenes-pkg-present "$pkg"; then
...
if $NEEDS_MIGRATION; then
  ...
  eratosthenes-pkg-drop walker-bin walker-bin-debug

  eratosthenes-pkg-add "${PACKAGES[@]}"

  source $ERATOSTHENES_PATH/install/config/walker-elephant.sh

  rm -rf ~/.config/walker/themes

  eratosthenes-refresh-config walker/config.toml
  eratosthenes-refresh-config elephant/calc.toml
  eratosthenes-refresh-config elephant/desktopapplications.toml
fi
