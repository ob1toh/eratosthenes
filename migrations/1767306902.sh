...
# Move user-added backgrounds from Eratosthenes theme folders to user config
ERATOSTHENES_DIR="$HOME/.local/share/eratosthenes"
USER_BACKGROUNDS_DIR="$HOME/.config/eratosthenes/backgrounds"

if [[ -d $ERATOSTHENES_DIR/themes ]]; then
  cd "$ERATOSTHENES_DIR"

  # Get list of git-tracked background files (relative to eratosthenes dir)
...
THEMES_DIR="$HOME/.config/eratosthenes/themes"
CURRENT_THEME_LINK="$HOME/.config/eratosthenes/current/theme"
...
elif [[ -f $HOME/.config/eratosthenes/current/theme.name ]]; then
  CURRENT_THEME_NAME=$(cat "$HOME/.config/eratosthenes/current/theme.name")
fi

# Remove all symlinks from ~/.config/eratosthenes/themes
...
if [[ -n $CURRENT_THEME_NAME ]]; then
  eratosthenes-theme-set "$CURRENT_THEME_NAME"
else
  # Backup to ensure a theme is set if we can't deduce the name
  eratosthenes-theme-set "Tokyo Night"
fi
