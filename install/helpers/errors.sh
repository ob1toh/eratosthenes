# No QR code branding needed for eratosthenes
...
show_log_tail() {
  if [[ -f $ERATOSTHENES_INSTALL_LOG_FILE ]]; then
    local log_lines=20
    local max_line_width=80

    tail -n $log_lines "$ERATOSTHENES_INSTALL_LOG_FILE" | while IFS= read -r line; do
...
catch_errors() {
  ...
  gum style --foreground 1 --padding "1 0 1 $PADDING_LEFT" "Eratosthenes installation stopped!"
  show_log_tail

  gum style "This command halted with exit code $exit_code:"
  show_failed_script_or_command

  gum style "Get help by opening an issue at https://github.com/ob1toh/eratosthenes/issues"
  ...
  while true; do
    ...
    case "$choice" in
    "Retry installation")
      bash "$ERATOSTHENES_PATH/install.sh"
      break
      ;;
    "View full log")
      if command -v less &>/dev/null; then
        less "$ERATOSTHENES_INSTALL_LOG_FILE"
      else
        tail "$ERATOSTHENES_INSTALL_LOG_FILE"
      fi
      ;;
